...
- **수용 기준** (시나리오 1 후반부 + 1-a + 5의 성공 기준 + wireframe screen-2/5의 진행률 시각 요소 파생):
  - [x] 5곡 모두 체크된 상태에서 "다운로드 시작" → 진행 화면에 카드 5개 표시
  - [x] 5곡 중 2곡 체크 해제 + "다운로드 시작" → 진행 화면에 카드가 정확히 3개 표시 (체크된 곡만) — 3곡 중 1곡 해제 → 카드 2개 검증
  - [x] 각 카드가 시간 경과에 따라 "완료" 또는 "실패+사유" 종결 상태로 이동한다
  - [x] 일부 곡이 실패해도 다른 곡의 변환은 정상 진행되어 종결에 도달한다 (unit test)
  - [x] 실패한 카드에 비어 있지 않은 사유 텍스트(예: "YouTube에서 못 찾음", "변환 실패")가 표시된다 (unit test)
  - [x] 종결 후 다운로드 폴더에 "완료" 상태인 곡 수만큼의 mp3 파일이 있다 — Titanium·Wake Me Up 2개 확인
  - [x] 진행 중 헤더에 "완료 N · 실패 N · 진행 중 N" 형식의 카운트와 진행률 바가 표시된다 (wireframe screen-2)
  - [x] 모든 카드가 종결 상태가 되면 헤더 텍스트가 "변환 진행"에서 "변환 완료"로 바뀌고 진행률 바가 100%로 표시된다 (wireframe screen-5)
  - [x] 모든 카드 종결 후 "새 입력으로" 버튼이 활성화되고 클릭 시 입력 화면(초기 상태)으로 복귀한다
  - [x] 변환 진행 중 다른 입력(취향 모드의 "곡 목록 만들기" 또는 URL 모드의 "변환") 시도 시 입력 버튼이 비활성 상태로 보인다 (mode 전환으로 InputScreen 자체 비표시)
- **검증**:
