...
- **수용 기준** (시나리오 2 성공 기준 + 불변 규칙 §새로고침·§동시 실행 제한 파생):
  - [ ] 입력 화면 하단에 "YouTube 영상 URL" 라벨이 붙은 입력란과 "변환" 버튼이 보인다
  - [ ] 유효한 URL을 입력하고 "변환"을 클릭하면 진행 화면으로 전환된다
  - [ ] 진행 화면에 카드가 정확히 1개 표시된다
  - [ ] 카드의 상태 텍스트가 시간 경과에 따라 "완료" 또는 "실패"로 이동한다
  - [ ] "완료" 상태일 때 클라이언트의 기본 다운로드 폴더에 mp3 1개가 저장된다
  - [ ] 모든 카드가 종결되기 전까지 "새 입력으로" 버튼은 비활성 상태로 보인다
  - [ ] 진행 화면 상태에서 페이지 새로고침 시 입력 화면(초기 상태)으로 복귀하고, 진행 중이던 카드 목록은 복구되지 않는다 (불변 규칙 §새로고침)
  - [ ] 변환 진행 중에는 입력 화면의 모든 제출 버튼이 비활성 상태로 보인다 (불변 규칙 §동시 실행 제한, URL 모드 시점에서 첫 검증)
- **검증**:
...
- **수용 기준** (시나리오 1·3·4·6·7의 성공 기준 + 불변 규칙 §동시 실행 제한 파생):
  - [ ] 입력 화면 상단에 취향 텍스트박스와 곡 수 input(기본값 10)이 보인다
  - [ ] 취향이 비어 있을 때 "곡 목록 만들기" 버튼이 비활성 상태로 보인다
  - [ ] 곡 수에 0 또는 51을 입력하면 값이 1·50으로 클램프되거나 제출이 막힌다
  - [ ] 유효 입력 + "곡 목록 만들기" 클릭 → 곡 목록 확인 화면 전환 + N개 항목 표시
  - [ ] 각 항목에 비어 있지 않은 곡명·아티스트·썸네일 이미지·외부 YouTube URL이 표시된다
  - [ ] 각 항목의 체크박스가 초기에 체크된 상태다
  - [ ] "미리 듣기" 링크 클릭 → 새 탭에서 해당 영상이 열린다 (`target="_blank"`, `rel="noopener"`)
  - [ ] 체크박스 토글 → 헤더의 "N곡 중 M곡 선택됨" 카운트와 "다운로드 시작 (M곡)" 라벨이 실시간 갱신
  - [ ] 체크 해제된 항목은 시각적으로 흐리게 표시되고 곡명에 취소선이 들어간다 (wireframe screen-3과 일치)
  - [ ] 모든 항목 체크 해제 → "다운로드 시작" 버튼이 비활성 상태로 보인다
  - [ ] "다시 생성" 클릭 → 새 항목 목록이 표시되고, 최소 한 곡 이상 직전 목록과 다르다
  - [ ] "입력 수정" 클릭 → 입력 화면 복귀 + 취향 텍스트·곡 수가 직전 입력값으로 채워져 있다
  - [ ] LLM 호출 실패 시 입력 화면을 유지한 채 비어 있지 않은 에러 메시지 Alert이 표시되고 입력값은 보존된다
  - [ ] LLM 호출 실패 후 "곡 목록 만들기" 버튼의 라벨이 "다시 시도"로 표시된다 (wireframe screen-7)
  - [ ] 곡 목록 생성 호출이 진행 중인 동안에는 취향의 "곡 목록 만들기"와 URL의 "변환" 버튼이 모두 비활성 상태로 보인다 (불변 규칙 §동시 실행 제한)
- **검증**:
